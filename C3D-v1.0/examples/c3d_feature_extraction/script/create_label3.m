...
% read split 1
fileID = fopen('UCF101_labels/trainlist03.txt');
C = textscan(fileID,'%s %d');
C = C(1);
C_tr = C{1};
fclose(fileID);

fileID = fopen('UCF101_labels/testlist03.txt');
C = textscan(fileID,'%s');
C_te = C{1};
fclose(fileID);


label_split3 = zeros(1, length(C_tr) + length(C_te));   % index of the video class
istrain_split3 = zeros(1, length(C_tr) + length(C_te)); % 1 if the video is train set and 0 otherwise
...
for m=1 : length(cnames)
    ...
    for v=1 : length(vnames)
        ...
        if ind > 0
            istrain_split3(counter) = 1;
        else
            istrain_split3(counter) = 0;
        end
        label_split3(counter) = clsi;
    end
end

save('/home/giuliat3/C3D/C3D-v1.0/examples/c3d_feature_extraction/output/UCF101_labels3.mat', 'istrain_split3', 'label_split3');

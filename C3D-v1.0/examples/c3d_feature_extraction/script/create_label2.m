...
% read split 2
fileID = fopen('UCF101_labels/trainlist02.txt');
C = textscan(fileID,'%s %d');
C = C(1);
C_tr = C{1};
fclose(fileID);

fileID = fopen('UCF101_labels/testlist02.txt');
C = textscan(fileID,'%s');
C_te = C{1};
fclose(fileID);


label_split2 = zeros(1, length(C_tr) + length(C_te));   % index of the video class
istrain_split2 = zeros(1, length(C_tr) + length(C_te)); % 1 if the video is train set and 0 otherwise
...
for m=1 : length(cnames)
    ...
    for v=1 : length(vnames)
        ...
        if ind > 0
            istrain_split2(counter) = 1;
        else
            istrain_split2(counter) = 0;
        end
        label_split2(counter) = clsi;
    end
end

save('/home/giuliat3/C3D/C3D-v1.0/examples/c3d_feature_extraction/output/UCF101_labels2.mat', 'istrain_split2', 'label_split2');
